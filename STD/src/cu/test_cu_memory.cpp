...
int main(int argc, char **argv) {
  ...
  std::cout << "size of a binary: " << sizeof(BinaryDescriptor)
            << ", size of a binaryF: " << sizeof(BinaryDescriptorF)
            << ", size of a STD: " << sizeof(STD)
            << ", size of a STDF: " << sizeof(STDF) << std::endl;
  getchar();
  ...
  BOOST_FOREACH (rosbag::MessageInstance const m, view) {
    ...
    if (is_build_descriptor) {
      ...
      std::cout << "Binary size: " << binary_list.size()
                << ", std size: " << STD_list.size() << ", memory: "
                << STD_list.size() * sizeof(STD) * 1.0 / (1024 * 1024) << " Mb"
                << std::endl;
      debug_file << "Binary size" << binary_list.size()
                 << ", std size:" << STD_list.size() << std::endl;
      all_std_size += STD_list.size();
      ...
      // }
      all_cloud_size += current_key_cloud->size();
      std::cout << "ds point size: " << current_key_cloud->size()
                << ", memory: "
                << key_cloud_list.back()->size() * sizeof(pcl::PointXYZI) *
                       1.0 / (1024 * 1024)
                << " Mb" << std::endl;
      key_frame_id++;
  ...
  std::cout << "all std size: " << all_std_size << " , std database memory:"
            << all_std_size * sizeof(STD) * 1.0 / (1024 * 1024) << " Mb"
            << std::endl;
  std::cout << "all cloud size: " << all_cloud_size << " , point cloud memory:"
            << all_cloud_size * sizeof(pcl::PointXYZI) * 1.0 / (1024 * 1024)
            << " Mb" << std::endl;
  std::cout << "push getchar to return" << std::endl;
  getchar();
  return 0;
}